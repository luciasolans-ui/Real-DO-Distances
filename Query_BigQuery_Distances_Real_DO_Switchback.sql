...
universe_sp AS (
   -- 7. Agrupamiento, mapeo de flotas e identificación de verticales y tiers horarios
   SELECT
     ...
     END AS vertical,
     CASE 
       WHEN EXTRACT(HOUR FROM ro.created_at_local) BETWEEN 6 AND 11 THEN 'Morning'
       WHEN EXTRACT(HOUR FROM ro.created_at_local) BETWEEN 12 AND 15 THEN 'Lunch'
       WHEN EXTRACT(HOUR FROM ro.created_at_local) BETWEEN 16 AND 19 THEN 'Afternoon'
       WHEN EXTRACT(HOUR FROM ro.created_at_local) BETWEEN 20 AND 23 THEN 'Dinner'
       ELSE 'Post Dinner'
     END AS time_tier,
